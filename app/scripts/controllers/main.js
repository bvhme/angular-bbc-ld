...
angular.module('angularBbcLdApp')
  .controller('MainCtrl', ['$scope', 'juicer', 'things', '$log', function ($scope, juicer, things, $log) {
    $scope.responses = [];
    var success = function (data) { $log.info(data); $scope.responses.push(data) };
    var fail = function (data) { $log.error(data); $scope.responses.push(data) };
    juicer.getSources('sun').then(success, fail);
    juicer.getArticles({'query': 'Sun'}).then(success, fail);
